...
//     console.log("[CONNECTION] Raw userId from frontend:", userId, "type:", typeof userId);

//     if (!userId || userId === "null" || userId === "undefined" || userId === "") {
//       console.log("[CONNECTION] Invalid userId → disconnecting socket");
//       socket.disconnect();
//       return;
//     }
...
//     console.log("[CONNECTION] Cleaned userId:", userId);

//     // Join room with cleaned string
//     socket.join(userId);
//     console.log(`[JOIN SUCCESS] User ${userId} joined room "${userId}" | Socket ID: ${socket.id}`);
...
//         status: "online",
//       });
//       console.log(`[STATUS] User ${userId} marked online`);
//     } catch (err) {
...
//             // Unread count nikaal lo (already calculate kiya hai upar)
...
//             // 1. Database mein notification banao (permanent)
//             const messageNotif = await Notification.create({
//               userId: otherParticipant._id,          // receiver
//               type: "message",
//               title: "New Message",
//               message: `${senderName} sent you a message`,   // ya zyada detailed: "sent: ${text.slice(0,40)}..."
//               senderId: senderId,
//               receiverId: otherParticipant._id,
//               relatedId: message._id,                // message ID link (future mein useful)
//               priority: "normal",
//               redirectUrl: `/messages/${chatId}`,    // ya `/chat/${chatId}` — jo bhi tera route hai
//             });

//             // 2. Populate karke frontend ke liye taiyar karo (same format jo getMyNotifications deta hai)
//             const populatedNotif = await Notification.findById(messageNotif._id)
//               .populate('senderId', 'name profilePicture');

//             // 3. Real-time push karo receiver ke room mein
//             io.to(otherParticipant._id.toString()).emit('newNotification', populatedNotif);

//             // Optional: agar tu grouped style (jaise WhatsApp) chahta hai to extra emit kar sakta hai
//             // io.to(otherParticipant._id.toString()).emit('messageNotificationUpdate', {
//             //   senderId: senderId.toString(),
//             //   senderName,
//             //   senderImage: sender?.profileImage,
//             //   messageCount: unreadCount,
//             //   chatId: chatId.toString(),
//             //   latestMessage: text || file?.name || '',
//             //   createdAt: new Date()
//             // });

//           } catch (notifError) {
//             console.error('Error creating/sending message notification:', notifError);
//           }
//         }
...
//               senderId: senderId,
//               receiverId: otherParticipant._id,
//               relatedId: message._id,                 // message ID se link kar do
//               priority: "normal",
//               redirectUrl: `/messages/${chatId}`,     // ya `/chat/${chatId}`
//             });
//           } catch (notifError) {
...
//           isSeen: false,
//         });
//         io.to(receiverId.toString()).emit("requestUpdated", {
//           totalPending,
//           // optional: event: "new-request"
//         });
//         console.log(`[REQ] Emitted requestUpdated with count ${totalPending} to ${receiverId}`);
//       } catch (err) {
  ...
  io.on("connection", async (socket) => {
    ...
    socket.on("sendMessage", async ({ chatId, senderId, text, file, replyTo, tempId }) => {
      try {
        ...
        // Sidebar update (must come before notification so we have updated participants)
        const updatedChat = await Chat.findById(chatId)
          .populate("participants", "name profileImage isOnline lastSeen")
          .populate({
            path: 'lastMessage',
        ...
        // Send notification to other participant (only once, after updatedChat)
        const otherParticipant = updatedChat.participants.find(p => p._id.toString() !== senderId.toString());
        if (otherParticipant) {
          try {
            const sender = await User.findById(senderId).select('name profileImage');
            ...
            // Better preview for notification
            ...
            const messageNotif = await Notification.create({
              userId: otherParticipant._id,          // receiver
              type: "message",
              title: unreadCount > 1 ? `New Messages (${unreadCount})` : "New Message",
              message: notifMessage,
              senderId: senderId,
              receiverId: otherParticipant._id,
              relatedId: message._id,                // link to this message
              priority: "normal",
              redirectUrl: `/messages/${chatId}`,    // adjust if your route is different e.g. /chat/${chatId}
            });

            // Populate for consistent frontend format
            const populatedNotif = await Notification.findById(messageNotif._id)
              .populate('senderId', 'name profilePicture');

            // Real-time emit → frontend modal update without refresh
            io.to(otherParticipant._id.toString()).emit('newNotification', populatedNotif);
    ...
    // --- Baaki sab events bilkul same rahe (no changes) ---
    ...
    socket.on('editMessage', async ({ messageId, newText, chatId, userId }) => {
      try {
        ...
        const fullChat = await Chat.findById(chatId)
          .populate({ path: 'lastMessage', select: 'text file createdAt sender isDeleted seen isEdited' })
          .populate('participants', 'name profileImage isOnline lastSeen');
        if (fullChat) {
          fullChat.participants.forEach(participant => {
            io.to(participant._id.toString()).emit('sidebarUpdate', fullChat);
