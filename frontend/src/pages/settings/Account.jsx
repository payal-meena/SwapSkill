...
import { UserCog, Trash2, Lock, ShieldCheck, Camera, Github, ChevronDown, ChevronUp, Instagram, Facebook, Ghost, Twitter, ExternalLink, Edit3, Check, X } from 'lucide-react';
...
const Account = () => {
  ...
  const [name, setName] = useState(""); // Name state add ki gayi hai seed ke liye
  const [gender, setGender] = useState("male"); // Gender state add ki gayi hai
  ...
  const [socialLinks, setSocialLinks] = useState({
    instagram: '', facebook: '', snapchat: '', github: '', twitter: ''
  });
  ...
  // Function to generate consistent Avatar based on gender and name
  const getAvatarUrl = (userGender, userName) => {
    const seed = userName || "User";
    // Agar gender female hai toh 'micah' style use hoga, warna 'avataaars' (male-leaning)
    const style = userGender === "female" ? "micah" : "avataaars";
  ...
  useEffect(() => {
    const fetchProfile = async () => {
      try {
        ...
        // Agar DB mein image hai toh wahi dikhao, warna consistent avatar generate karo
        if (data.profileImage) {
          setPreviewImage(data.profileImage);
        } else {
          setPreviewImage(getAvatarUrl(data.gender, data.name));
        }
  ...
  const handleUpdateProfile = async () => {
    setIsUpdating(true);
    try {
      await api.put('/users/me', { email, bio, profession, gender, socialLinks });
      // Update preview if no custom image is set
      if (!profileImage && !previewImage.includes('cloudinary')) {
          setPreviewImage(getAvatarUrl(gender, name));
      }
      showModal('success', 'Profile Updated', 'Your changes have been saved.');
      setActiveTab(null); 
    } catch (error) {
  ...
  const handleDeleteProfileImage = async () => {
    ...
    try {
      ...
      setPreviewImage(getAvatarUrl(gender, name)); // Reset to consistent default
      setProfileImage(null);
      showModal('success', 'Profile Image Removed', 'Default avatar restored.');
      setActiveTab(null);
    } catch (error) {
      showModal('error', 'Image Delete Failed', "Failed to delete profile image.");
    }
    setIsUpdating(false);
  };
  ...
  const deleteAccountFinal = async () => {
    try {
      ...
      showModal('error', 'Action Failed', 'Could not delete account.');
    }
  };

  const confirmDelete = () => {
    showModal('delete', 'Delete Account?', 'Proceed with permanent deletion?', deleteAccountFinal);
  };
  ...
  return (
    <div className="w-full max-w-6xl mx-auto px-4 md:px-8 py-10 space-y-8 animate-in fade-in duration-700 font-['Lexend']">
      ...
      <div className="mb-4">
        ...
        <p className="text-slate-500 dark:text-[#92c9a4]">Manage your identity and security.</p>
      </div>
      ...
      <div className="space-y-4">
        <div className="bg-white dark:bg-[#112217] rounded-3xl border border-slate-200 dark:border-[#23482f] overflow-hidden">
          <button onClick={() => toggleTab('general')} className="w-full flex items-center justify-between p-7 hover:bg-slate-50 dark:hover:bg-[#1a3323]">
            <div className="flex items-center gap-5">
              ...
              <div className="text-left"><span className="block font-bold text-lg dark:text-white">General Settings</span><span className="text-xs text-slate-500">Identity and Info</span></div>
            </div>
          ...
          <div className={`transition-all duration-300 ${activeTab === 'general' ? 'max-h-[1000px] p-8 pt-2 opacity-100' : 'max-h-0 overflow-hidden opacity-0'}`}>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-8 mb-6">
              <div className="space-y-2">
                <label className="text-xs font-bold text-slate-400 uppercase tracking-wider ml-1">Gender (For Default Avatar)</label>
                <select 
                  value={gender} 
                  onChange={(e) => setGender(e.target.value)} 
                  className="w-full px-5 py-3.5 rounded-2xl bg-slate-100 dark:bg-[#23482f] text-slate-900 dark:text-white outline-none appearance-none"
                >
              ...
              <div className="space-y-2">
                ...
                <input type="text" value={profession} onChange={(e) => setProfession(e.target.value)} className="w-full px-5 py-3.5 rounded-2xl bg-slate-100 dark:bg-[#23482f] text-white outline-none" placeholder="e.g. Designer" />
              </div>
            </div>
        ...
        {/* Security Section */}
        <div className="bg-white dark:bg-[#112217] rounded-3xl border border-slate-200 dark:border-[#23482f] overflow-hidden">
          <button onClick={() => toggleTab('security')} className="w-full flex items-center justify-between p-7 hover:bg-slate-50 dark:hover:bg-[#1a3323]">
            <div className="flex items-center gap-5">
              <div className="p-3 bg-slate-100 dark:bg-[#23482f] rounded-2xl"><Lock className="text-[#13ec5b]" size={24} /></div>
              <div className="text-left"><span className="block font-bold text-lg dark:text-white">Security</span><span className="text-xs text-slate-500">Manage Password</span></div>
            </div>
            {activeTab === 'security' ? <ChevronUp size={24} /> : <ChevronDown size={24} />}
          </button>
          <div className={`transition-all duration-300 ${activeTab === 'security' ? 'max-h-[800px] opacity-100 p-8 pt-2' : 'max-h-0 opacity-0 overflow-hidden'}`}>
            ...
            <button disabled={!canSavePassword} onClick={handleSavePassword} className="px-10 py-3.5 bg-[#13ec5b] text-[#102216] font-bold rounded-xl disabled:opacity-30">Update Password</button>
          </div>
        </div>

        {/* Social Presence */}
        <div className="bg-white dark:bg-[#112217] rounded-3xl border border-slate-200 dark:border-[#23482f] overflow-hidden">
          <button onClick={() => toggleTab('connections')} className="w-full flex items-center justify-between p-7 hover:bg-slate-50 dark:hover:bg-[#1a3323]">
            <div className="flex items-center gap-5">
              <div className="p-3 bg-slate-100 dark:bg-[#23482f] rounded-2xl"><ShieldCheck className="text-[#13ec5b]" size={24} /></div>
              <div className="text-left"><span className="block font-bold text-lg dark:text-white">Social Presence</span><span className="text-xs text-slate-500">Linked Accounts</span></div>
            </div>
            {activeTab === 'connections' ? <ChevronUp size={24} /> : <ChevronDown size={24} />}
          </button>
          <div className={`transition-all duration-300 ${activeTab === 'connections' ? 'max-h-[1200px] opacity-100 p-8 pt-2' : 'max-h-0 opacity-0 overflow-hidden'}`}>
                ...
                <div key={social.id} className="p-4 rounded-2xl border border-slate-100 dark:border-[#23482f] bg-slate-50/50 dark:bg-[#1a3323]/20 flex flex-col gap-3">
                  <div className="flex items-center justify-between">
                    ...
                    <div className="flex items-center gap-2">
                      <button onClick={() => handleConnect(socialLinks[social.id])} className="flex items-center gap-1 px-3 py-1.5 bg-[#13ec5b]/10 text-[#13ec5b] rounded-lg text-xs font-bold hover:bg-[#13ec5b] hover:text-[#102216] transition-all"><ExternalLink size={14} /> Connect</button>
                      <button onClick={() => startEdit(social.id, socialLinks[social.id])} className="p-1.5 text-slate-400 hover:text-[#13ec5b] transition-colors"><Edit3 size={18} /></button>
                    </div>
                  </div>
                  {editingField === social.id && (
                    <div className="flex items-center gap-2 mt-1">
                      <input type="text" value={tempValue} onChange={(e) => setTempValue(e.target.value)} placeholder="URL..." className="flex-1 px-3 py-2 bg-white dark:bg-[#23482f] dark:text-white rounded-lg text-xs outline-none border border-[#13ec5b]/30" />
                  ...
                  {!editingField && socialLinks[social.id] && <p className="text-[10px] text-slate-400 truncate px-1 italic">{socialLinks[social.id]}</p>}
                </div>
              ))}
            </div>
            <div className="flex justify-end pt-4 border-t border-slate-100 dark:border-[#23482f]">
                <button onClick={handleUpdateProfile} disabled={isUpdating} className="px-8 py-3 bg-[#13ec5b] text-[#102216] font-bold rounded-xl active:scale-95 shadow-lg shadow-[#13ec5b]/20">{isUpdating ? 'Saving...' : 'Save All Changes'}</button>
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="pt-8 border-t border-slate-200 dark:border-[#23482f] flex flex-col md:flex-row gap-4 justify-between items-center">
        <p className="text-sm text-slate-500">Dangerous area: Action cannot be undone.</p>
        <button onClick={confirmDelete} className="flex items-center gap-2 px-8 py-4 rounded-2xl border border-red-500/30 text-red-500 hover:bg-red-500/10 transition-all text-sm font-bold">
